...
3. Add `TMDB_READ_TOKEN` as a Railway service variable. Never put it in the image
   or repository.
4. Set a strong `WANNA_WATCH_PASSWORD` service variable. The login page uses this
   shared password. Railway startup refuses to run without a password.
...
Password authentication protects the app, static assets, API, and API documentation.
The login page, its styling assets, and `/healthz` are public. Always use HTTPS
...
Enter the shared password once per browser to stay signed in for 30 days, including
browser restarts and deployments. Sessions have a fixed expiry; sign in again after
30 days. Sign out clears this browser’s cookie. Changing the Railway password and
redeploying invalidates all sessions. Private browsing and clearing cookies can end
a session sooner. Login uses an HttpOnly, SameSite cookie, marked Secure over HTTPS.
